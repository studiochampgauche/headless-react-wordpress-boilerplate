...
- Add the attribute `data-transition="true"` to your `<Link>` or `<a ...>` elements. Without this attribute, the transition will be direct.

- If you want to use a custom HTML element instead of `<Link>` or `<a ...>`, you need to add the class name `goto` with the attribute `data-href=""`.

- If your `data-href` attribute is an anchor (or your `href` attribute from `<a ...>` element too) , you need to include the path of your current page if you want to stay on the same page and scroll to the anchor (e.g., if your page is your home page, you should add `/` with your anchor, like `/#my-id`). You don't need to do this with `<Link>`.
...
- Using `mailto:`, `tel:`, or other schemas/protocols that are not `http` or `https` may not work with the `<Link>` or `<a>` elements. To resolve this issue, set `target="_self"`.  (Yeah, it's the default, but when you use target, you use the element as default, not the element managed by `PageTransition.jsx`)

- Use `<Wrapper value={my_value} />` for place HTML when you can't use `dangerouslySetInnerHTML`.

- If you prefer using `SASS` instead of `SCSS`, you only need to change your `.scss` extension to `.sass`. (And remove all brackets of course)
