...
### Call an ACF Field
Use `scg::field($field_name, $id)` or `StudioChampGauche\Utils\Field::get($field_name, $id)` to call an ACF Field. `$field_name` is required. If you do not enter the `$id` parameter, the function will first look in 'option' and then look at the current page or the current looped element.
